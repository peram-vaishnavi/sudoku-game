...
const checkPlayerMove = (grid, sudoku, row, col) => {
    if (grid[row][col] == sudoku[row][col]) {
        return true;
    } else {
        lives--;
        alert(`Incorrect move! You have ${lives} lives left.`);
...
const checkGameStatus = () => {
    ...
    let col = index % CONSTANT.GRID_SIZE;
    console.log(row);
    let isCorrectMove = checkPlayerMove(su_answer, su.original, row, col);
    let isGridFull = isFullGrid(su_answer);
    let areGridsDifferent = gridsAreDifferent(su.original, su_answer);
    
    console.log(isCorrectMove);
    console.log(isGridFull);
    console.log(areGridsDifferent);
    
    if (isCorrectMove && isGridFull) {
        return 'win';
    } else if ((!isCorrectMove || isGridFull) && areGridsDifferent) {
        return 'lose';
    } else {
        return 'in-progress';
    }
...
const initNumberInputEvent = () => {
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (key >= 1 && key <= 9 && selected_cell !== -1) {
            if (!cells[selected_cell].classList.contains('filled')) {
                fillCellWithValue(key);
            }
        }
    });
};
